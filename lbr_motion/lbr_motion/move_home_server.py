...
class MoveHomeServer(Node):
    def __init__(self):
        ...
        if self.robot_name == 'med14':
            ...
            self.get_logger().info('MvHome SRV: Robot name set to "med14"')
            ...
            self.get_logger().info('MvHome SRV: Robot name set to "med14_tc"')
            ...
            self.get_logger().info('MvHome SRV: Robot name set to "med14_robotiq_2f"')

        else:
            self.get_logger().warn('MvHome SRV: No valid robot name specified in MoveToGoal() initialization, various topics/services/actions may not work!') 
            self.planning_gropus = ['arm']
            self.joint_names = {
                'arm': ['lbr_A1', 'lbr_A2', 'lbr_A3', 'lbr_A4', 'lbr_A5', 'lbr_A6', 'lbr_A7'],
                'all': ['lbr_A1', 'lbr_A2', 'lbr_A3', 'lbr_A4', 'lbr_A5', 'lbr_A6', 'lbr_A7']
        ...
        #     self.get_logger().info('MvHome SRV: IK service not available, waiting for IK service...')
        # self.get_logger().info("MvHome SRV: IK service has been started")

        ############################################
        # MvHome WRAPPER ACTION SERVER (MOVE_TO_POSE)
        ############################################
        # Create the action server - this is what clients will connect to
        self._action_server = ActionServer(
            self,
            WrapperAction,  # The action type for this wrapper
            'move_to_home',  # The action name
        ...
        while not self._action_client.wait_for_server(timeout_sec=1.0):
            self.get_logger().info('MvHome SRV: Target action server (move_action) not available, waiting for target action server...')
        self.get_logger().info("MvHome SRV: Target action server (move_action) has been started")
        ...
        self.get_logger().info("MvHome SRV: End-effector pose listener has been started")
        ...
        self.get_logger().info("MvHome SRV: MoveToPose joint state subscriber has been started")

        # Dictionary to keep track of active target goal handle
        self._target_goal_handle = None
        
        self.get_logger().info('MvHome SRV: Wrapper action server has been started')
    ...
    # /***************************************************************************************************/
    # /*    MvHome GOAL_CALLBACK                                                                            */
    # /***************************************************************************************************/
    def goal_callback(self, goal_request):
        """Called when a goal is received from a client"""
        self.get_logger().info('MvHome SRV: Received goal request')
        
        # Use thread locking to check if a goal is already active
        with self._lock:
            if self._is_active:
                self.get_logger().warn('MvHome SRV: Rejecting goal: Another goal is already active')
                return GoalResponse.REJECT
            else:
                self.get_logger().info('MvHome SRV: Accepting goal')
                return GoalResponse.ACCEPT

    # /***************************************************************************************************/
    # /*    MvHome CANCEL_CALLBACK                                                                          */
    # /***************************************************************************************************/
    def cancel_callback(self, goal_handle):
        """Called when a cancel request is received"""
        self.get_logger().info('MvHome SRV: Received cancel request')
        ...
        with self._lock:
            ...
            if goal_handle == self._current_goal_handle:
                ...
                if self._target_goal_handle:
                    self.get_logger().info('MvHome SRV: Cancelling target goal')
    ...
    # /***************************************************************************************************/
    # /*    MvHome EXECUTE_CALLBACK                                                                         */
    # /***************************************************************************************************/
    async def execute_callback(self, goal_handle):
        """Called when a goal is accepted and should be executed"""
        self.get_logger().info('MvHome SRV: Executing goal...')
        ...
        try:
            ...
            joint_positions = [0.0] * len(self.joint_names[planning_group])
            ...
            target_goal = self.create_move_group_goal(joint_positions, planning_group, vel_scaling, acc_scaling)
            ...
            if not target_goal_handle.accepted:
                self.get_logger().error('MvHome SRV: Target server rejected the goal')
                goal_handle.abort()
            ...
            if target_result.status == GoalStatus.STATUS_SUCCEEDED:
                self.get_logger().info('MvHome SRV: Target action completed successfully')
                goal_handle.succeed()
            else:
                self.get_logger().warn(f'MvHome SRV: Target action failed with status: {target_result.status}')
                goal_handle.abort()
                
            return wrapper_result
        
        except Exception as e:
            self.get_logger().error(f'MvHome SRV: Failed to execute goal: {e}')
            goal_handle.abort()
...
def main(args=None):
    ...
    # Create node
    MvHome_action_server = MoveHomeServer()
    
    # Use MultiThreadedExecutor to allow concurrent callbacks
    executor = MultiThreadedExecutor()
    executor.add_node(MvHome_action_server)
    ...
    try:
        ...
        # Clean up
        executor.shutdown()
        MvHome_action_server.destroy_node()
        rclpy.shutdown()
